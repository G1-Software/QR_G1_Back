...
test("test hacer solicitud de forma correcta ", async ({ page }) => {
  await page.goto(
    "https://qr-uc-christus.app/?token=a3f5c2d8e9b14f6c8a0d3e4b9f2c1a7e"
  );
  await expect(
    page.getByRole("link", {
      name: "SOLICITUDES (LIMPIEZA, MANTENCIÓN, NUTRICIÓN, ETC)",
    })
  ).toBeVisible();
  await page
    .getByRole("link", {
    ...
    .click();
  await expect(
    page.getByRole("heading", { name: "ENVÍO DE SOLICITUD" })
  ).toBeVisible();
...
test("test hacer solicitud con mail erroneo por lo tanto no se envia", async ({
  page,
}) => {
  await page.goto(
    "https://qr-uc-christus.app/?token=a3f5c2d8e9b14f6c8a0d3e4b9f2c1a7e"
  ...
  await page
    .getByRole("combobox")
    .nth(1)
    .selectOption("Solicita Oraciones para su Salud");
  await page.locator("textarea").click();
...
test("test hacer solicitud sin llenar campos obligatorios de nombre y correo", async ({
  page,
}) => {
  await page.goto(
    "https://qr-uc-christus.app/?token=a3f5c2d8e9b14f6c8a0d3e4b9f2c1a7e"
  ...
  await page
    .getByRole("combobox")
    .nth(1)
    .selectOption("Solicita Sacramento, Comunión y Confesión");
  await page.locator("textarea").click();
...
test("test hacer solicitud sin ingresar nombre, por lo tanto no se envia", async ({
  page,
}) => {
  await page.goto(
    "https://qr-uc-christus.app/?token=a3f5c2d8e9b14f6c8a0d3e4b9f2c1a7e"
